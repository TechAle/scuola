...
Elenco delle filiali (descrizione) nelle quali ci sono più di 3 impiegati
</div>
<br>
<?php 
...
$query=<<<SQL
    SELECT descrizione, COUNT(Filiale) AS CONTEGGIO
    FROM personale,filiali
    WHERE Funzione='Impiegato' AND personale.Filiale=filiali.Codice
    GROUP BY Filiale
    HAVING COUNT(*) > 3;
    SQL;
